...
public final class LibsChecker {
	...
	//
	// public static final boolean checkVitamioLibs(Activity ctx, int msgID, int
	// rawID) {
	// new Vitamio().
	// if (!Vitamio.isInitialized(ctx) &&
	// !ctx.getIntent().getBooleanExtra(FROM_ME, false)) {
	// Intent i = new Intent();
	// i.setClassName(Vitamio.getCompatiblePackage(),
	// "com.nmbb.oplayer.ui.vitamio.InitActivity");
		...
		try {
			if ((!Vitamio.isInitialized(ctx))
					&& (!ctx.getIntent().getBooleanExtra(
							"fromVitamioInitActivity", false))) {
				Intent i = new Intent();
				i.setClassName(ctx.getPackageName(),
						"local.player.vi.widget.InitActivity");// io.vov.vitamio.activity.InitActivity
				i.putExtras(ctx.getIntent());
				i.setData(ctx.getIntent().getData());
