...
public class CircleProgressDown extends View {
	...
	public DownCircleCallBack callBack;
	...
	public CircleProgressDown(Context context, AttributeSet attrs) {
		super(context, attrs);
		...
		// mCircleAttribute.setPaintColor(40, 177, 255);// 设置默认画笔颜色为蓝色
		mCircleAttribute.setPaintColor(0x24, 0x93, 0xd2);// 设置默认画笔颜色为蓝色
		...
		mProgressCircle = r.getDrawable(R.drawable.dlna_volume_thumb);// 进度条的小圆球
	...
	private void initData() {
		gapDegree = (180 - mCircleAttribute.mDegree) / 2f;
	...
	public void onDraw(Canvas canvas) {
		...
		if (mCircleAttribute.mDirection.endsWith(UP)) {// 方向为上时
			...
			canvas.drawArc(mCircleAttribute.mRoundOval, gapDegree - 180,
			...
			canvas.drawArc(mCircleAttribute.mRoundOval, gapDegree,
					mCircleAttribute.mDegree,
					// mCircleAttribute.mBRoundPaintsFill,
					false, mCircleAttribute.mBottomPaint);
			sweep = -mCircleAttribute.mDegree * rate;
			canvas.drawArc(mCircleAttribute.mRoundOval,
					mCircleAttribute.mDrawPos, sweep,
					// mCircleAttribute.mBRoundPaintsFill,
					false, mCircleAttribute.mMainPaints); // 画进度条
	...
	class CircleAttribute {
		...
		public CircleAttribute() {
			...
			mMainPaints.setColor(Color.argb(0xff, 0x2e, 0xbd, 0xff));
		...
		public void setPaintColor(int red, int green, int blue) {
			mMainPaints.setColor(Color.rgb(red, green, blue));
				...
				if (mCircleAttribute.mDirection.equals(UP)) {
					...
					if (y < circleHeight) {
						if (x > circleWidth) {// 大于90度时
							...
							// 触摸点落在半圆内
							double degree = Math.toDegrees(Math.asin((event
									.getX() - circleWidth) / r));
							...
							mMainCurProgress = (int) ((Math.toDegrees(Math
									.asin((circleHeight - event.getY()) / r)) - gapDegree) / 180f * mMaxProgress);
							// }
						}
					}
				} else {
					if (y > circleHeight) {
						if (x > circleWidth) {// 大于90度时
							...
							// 触摸点落在半圆内
							mMainCurProgress = (int) ((Math.toDegrees(Math
									.asin((-circleHeight + event.getY()) / r)) - gapDegree)
									/ mCircleAttribute.mDegree * mMaxProgress);
							// }
						}
					}
	...
	public void setCallfuc(DownCircleCallBack cb) {
		this.callBack = cb;
	}

	public void call() {
		this.callBack.downCircleCallBack();
	}
	 ...
	 * 
	 */
	public interface DownCircleCallBack {
