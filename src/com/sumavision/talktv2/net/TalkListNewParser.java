	...
	public String parse(String s, VodProgramData p) {
		...
		try {
			...
			if (OtherCacheData.current().isDebugMode)
				Log.e("TalkTopicParser", "errorCode=" + errCode);

			if (errCode == JSONMessageType.SERVER_CODE_OK) {
				JSONObject jsonObject = jAData.getJSONObject("content");
				p.talkCount = jsonObject.getInt("talkCount");
				...
				p.setComment(lc);
				p.canShot = jsonObject.getInt("canShot");
				if (p.canShot != 0) {
					JSONObject screenShot = jsonObject
							.getJSONObject("screenShot");
					ScreenShotData.address = screenShot
							.getString("requestAddress");
					ScreenShotData.channelId = screenShot.getLong("channelId");
				}
			} else
